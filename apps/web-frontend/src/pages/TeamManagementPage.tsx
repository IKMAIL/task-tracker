...
export default function TeamManagementPage(): React.ReactElement {
  ...
  const set = (key: keyof TeamForm, val: string) => setForm((f) => ({ ...f, [key]: val }));
      ...
      {showForm && (
        <form className="form-card" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <label>Name *</label>
            <input value={form.name} onChange={(e) => set('name', e.target.value)} required minLength={2} maxLength={100} />
          </div>
          <div className="form-group">
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button type="submit" className="btn btn-primary" disabled={submitting}>
              {submitting ? 'Saving...' : editingId ? 'Update Team' : 'Create Team'}
            </button>
        ...
        <tbody>
          {(teams || []).map((team) => (
            <tr key={team._id}>
              ...
              {isAdmin && (
                <td>
                  <button className="btn btn-sm" onClick={() => openEdit(team)} style={{ marginRight: '0.5rem' }}>Edit</button>
                  <button className="btn btn-sm" onClick={() => handleDelete(team)} style={{ color: 'var(--danger)' }}>Delete</button>
                </td>
              )}
            </tr>
          ))}
          {(teams || []).length === 0 && (
            <tr><td colSpan={isAdmin ? 4 : 3} style={{ textAlign: 'center' }}>No teams found.</td></tr>
          )}
        </tbody>
      </table>
    </div>
