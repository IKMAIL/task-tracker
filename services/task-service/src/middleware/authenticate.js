...
// Validates the internal service-to-service token
exports.requireServiceToken = (req, res, next) => {
  if (req.headers['x-service-token'] !== process.env.SERVICE_TOKEN) {
    return res.status(403).json({ success: false, error: { message: 'Invalid service token' } });
  }
  next();
};
